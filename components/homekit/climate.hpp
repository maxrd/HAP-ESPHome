...
private:
  ...
  static int climate_read(hap_char_t* hc, hap_status_t* status_code, void* serv_priv, void* read_priv) {
    ...
    if (!strcmp(type, HAP_CHAR_UUID_CURRENT_HEATING_COOLING_STATE)) {
      ...
      float val = obj->current_temperature;
      if (!std::isfinite(val)) val = 25.0;
      state.f = val;
    } else if (!strcmp(type, HAP_CHAR_UUID_CURRENT_RELATIVE_HUMIDITY)) {
      float val = obj->current_humidity;
      if (!std::isfinite(val)) val = 50.0;
      state.f = val;
    }
  ...
  static int climate_write(hap_write_data_t write_data[], int count, void* serv_priv, void* write_priv) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!strcmp(type, HAP_CHAR_UUID_TARGET_HEATING_COOLING_STATE)) {
        ...
        float val = write->val.f;
        if (!std::isfinite(val)) val = 25.0;
        obj->make_call().set_target_temperature(val).perform();
      } else if (!strcmp(type, HAP_CHAR_UUID_TARGET_RELATIVE_HUMIDITY)) {
        float val = write->val.f;
        if (!std::isfinite(val)) val = 50.0;
        obj->make_call().set_target_humidity(val).perform();
      }
    }
    return HAP_SUCCESS;
  }
...
public:
  ...
  void setup() {  // 移除 override，避免編譯錯誤
    ...
    float ct = climatePtr->current_temperature;
    float tt = climatePtr->target_temperature;
    if (!std::isfinite(ct)) ct = 25.0;
    if (!std::isfinite(tt)) tt = 25.0;

    hap_serv_t* service = hap_serv_thermostat_create(current_mode, target_mode, ct, tt, CELSIUS);

    climate::ClimateTraits traits = this->climatePtr->get_traits();
    if (traits.get_supports_current_humidity()) {
      float hum = climatePtr->current_humidity;
      if (!std::isfinite(hum)) hum = 50.0;
      hap_serv_add_char(service, hap_char_current_relative_humidity_create(hum));
    }
    if (traits.get_supports_target_humidity()) {
      float hum = climatePtr->target_humidity;
      if (!std::isfinite(hum)) hum = 50.0;
      hap_serv_add_char(service, hap_char_target_relative_humidity_create(hum));
    }
